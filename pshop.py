...
class Photoshop:
    ...
    def query8(self,crsr):
        ...
        result = crsr.fetchall()
        for r in result:
            print('%s has a combined price of $%d' % (r[0],r[1]))

    def query9(self,crsr,mydb,photoID):
        crsr.execute("SELECT * FROM photo")
        result = crsr.fetchall()
        print("PhotoIDs before deleting: ")
        for r in result:
            print(r[0])
    ...
    def query10(self,crsr,mydb,photoID,name):
        crsr.execute("SELECT * FROM photo")
        result = crsr.fetchall()
        print("PhotoIDs and Photographers before updating: ")
        for r in result:
            print('%d , %s' % (r[0],r[9]))
        ...
        crsr.execute("SELECT * FROM photo")
        result = crsr.fetchall()
        print("PhotoIDs and Photographers after updating: ")
        for r in result:
            print('%d , %s' % (r[0],r[9]))
